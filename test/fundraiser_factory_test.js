...
contract("FundraiserFactory: deployment", () => {
    it("has been deployed", async () => {
        ...
        assert(fundraiserFactory, "fundraised factory was not deployed")
    });
});
...
contract("FundraiserFactory: fundraisers", (accounts) => {
    ...
    async function addFundraisers(factory, count, accounts) {
        ...
        for (let i = 0; i < count; i++) {
            await factory.createFundraiser(
                // create a series of fundraisers.
                // The index will be used to make each on unique.
                `${name} ${i}`,
                `${lowerCaseName}${i}.com`,
                `${lowerCaseName}${i}.png`,
    ...
    describe("varying limits", async () => {
        ...
        it("returns 20 results when the limit requested is 30", async () => {
